...
import { Fingerprint, Shield, Skull, Eye, Play, ArrowRight, Lock, Beer, Unlock, AlertCircle, Check, Radio, Network } from 'lucide-react';
...
export const IdentityCard: React.FC<Props> = ({ player, theme, color, onRevealStart, onRevealEnd, nextAction, readyForNext, isLastPlayer, isParty, partyIntensity, debugMode, onOracleConfirm }) => {
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        // Oracle Lock: If in selection mode, maybe prevent dragging? 
        // Let's allow dragging but ensure UI is usable.
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center w-full px-4 overflow-visible my-auto mb-8 relative">
                                    ...
                                    {player.isOracle && !oracleSelectionMade && !isTransmitting ? (
                                        <div className="w-full flex flex-col gap-2 animate-in fade-in zoom-in duration-300">
                                            ...
                                            <div className="grid gap-2">
                                                {oracleOptions.map((opt, i) => (
                                                    <button
                                                        key={i}
                                                        onPointerDown={(e) => { e.stopPropagation(); handleOracleOptionSelect(opt); }}
                                                        className="bg-violet-500/20 border border-violet-500/50 py-3 rounded hover:bg-violet-500/40 active:scale-95 transition-all text-sm font-bold text-violet-100 uppercase tracking-wide"
                                                    >
                                                        {opt}
                                                    </button>
                                                ))}
                                        ...
                                        <>
                                            ...
                                            {/* PROTOCOLO NEXUS DISPLAY (v6.5) */}
                                            {player.isImp && player.nexusPartners && player.nexusPartners.length > 0 && (
                                                <div className="mt-4 w-full bg-red-900/10 border border-red-500/30 rounded-lg p-3 animate-in fade-in slide-in-from-bottom duration-500">
                                                    <div className="flex items-center justify-between mb-2">
                                                        <div className="flex items-center gap-1.5">
                                                            <Network size={12} className="text-red-400 animate-pulse" />
                                                    ...
                                                    <p className="text-[9px] text-red-300 font-mono mb-1 opacity-70">
                                                        COMPAÑEROS IDENTIFICADOS:
                                                    </p>
                                                    <div className="flex flex-wrap gap-2">
                                                        {player.nexusPartners.map((partnerName, idx) => (
                                                            <div key={idx} className="flex items-center gap-1.5 bg-red-500/10 px-2 py-1 rounded border border-red-500/20">
